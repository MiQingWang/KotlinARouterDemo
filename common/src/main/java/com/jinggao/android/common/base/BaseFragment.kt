package com.jinggao.android.common.base
...
abstract class BaseFragment<V : ViewDataBinding?, VM : BaseViewModel<*>?> :
    ...
    override fun onCreateView(): View? {
        binding = DataBindingUtil.inflate<V>(
            LayoutInflater.from(requireContext()), initContentView(),
            null,
            false)
    ...
    private fun  initViewDataBinding() {
        ...
        if (viewModel == null) {
            ...
            modelClass = if (type is ParameterizedType) {
                (type as ParameterizedType).actualTypeArguments[1] as Class<*>
            } else {
    ...
    protected fun registorUIChangeLiveDataCallBack() {
        //加载对话框显示
        viewModel!!.getUC().showDialogEvent.observe(this, object : Observer<String?> {
            override fun onChanged(@Nullable title: String?) {
                showDialog(title)
            }
        })
        //加载对话框消失
        viewModel!!.getUC().dismissDialogEvent
        ...
        viewModel!!.getUC().startActivityEvent.observe(this,
            object : Observer<Map<String?, Any?>?> {
                override fun onChanged(t: Map<String?, Any?>?) {
                    val clz =
                        t?.get(ParameterField.CLASS) as Class<*>?
                    val bundle = t?.get(ParameterField.BUNDLE) as Bundle?
        ...
        //跳入ContainerActivity
        viewModel!!.getUC().startContainerActivityEvent.observe(
            this,
            object : Observer<Map<String?, Any?>?> {
                override fun onChanged(t: Map<String?, Any?>?) {
                    val canonicalName =
                        t?.get(ParameterField.CANONICAL_NAME) as String?
                    val bundle = t?.get(ParameterField.BUNDLE) as Bundle?
        ...
        //关闭界面
        viewModel!!.getUC().finishEvent.observe(this, object : Observer<Void?> {
            override fun onChanged(@Nullable v: Void?) {
                activity!!.finish()
            }
        })
        //关闭上一层
        viewModel!!.getUC().onBackPressedEvent
            .observe(this, object : Observer<Void?> {
                override fun onChanged(@Nullable v: Void?) {
    ...
    fun showDialog(title: String?) {
        ...
        dialog = QMUITipDialog.Builder(context)
